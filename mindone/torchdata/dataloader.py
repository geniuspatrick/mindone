...
# Ideally we would parameterize `DataLoader` by the return type of `collate_fn`, but there is currently no way to
# have that type parameter set to a default value if the user doesn't pass in a custom 'collate_fn'.
        ...
        if multiprocessing_context is not None:
            if self.num_workers > 0:
                if not isinstance(multiprocessing_context, python_multiprocessing.context.BaseContext):
                    raise TypeError(f'multiprocessing_context option should be a valid context '
                                    f'object, but got multiprocessing_context={multiprocessing_context}')
            else:
...
class _BaseDataLoaderIter:
    def __init__(self, loader: DataLoader) -> None:
        ...
        if loader.generator is None:
            self._base_seed = np.random.Generator(np.random.PCG64()).integers(int(-2**63), int(2**63-1)).item()
        else:
            self._base_seed = loader.generator.integers(int(-2**63), int(2**63-1)).item()
    ...
    def __next__(self) -> Any:
        # TODO: We can do profiling here like `with torch.autograd.profiler.record_function(self._profile_name)`.
