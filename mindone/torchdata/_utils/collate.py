...
def default_convert(data):
    r"""
    Convert each element into a :class:`numpy.ndarray`.

    If the input is a `Sequence`, `Collection`, or `Mapping`, it tries to convert each element inside to a :class:`numpy.ndarray`.
...
def collate(batch, *, collate_fn_map: Optional[Dict[Union[Type, Tuple[Type, ...]], Callable]] = None):
    ...
    Examples:
        ...
        ...     collate_map = {np.ndarray: collate_tensor_fn}
        ...     return collate(batch, collate_fn_map=collate_map)
        >>> # Extend `default_collate` by in-place modifying `default_collate_fn_map`
        >>> default_collate_fn_map.update({np.ndarray: collate_tensor_fn})
...
def default_collate(batch):
    ...
    The exact output type can be a :class:`numpy.ndarray`, a `Sequence` of :class:`numpy.ndarray`, a
    Collection of :class:`numpy.ndarray`, or left unchanged, depending on the input type.
    This is used as the default function for collation when
    `batch_size` or `batch_sampler` is defined in :class:`~torch.utils.data.DataLoader`.

    Here is the general input type (based on the type of the element within the batch) to output type mapping:

        * :class:`numpy.ndarray` -> :class:`numpy.ndarray` (with an added outer dimension batch size)
    ...
    Examples:
        ...
        {'A': array([  0, 100]), 'B': array([  1, 100])}
        >>> # Example with `NamedTuple` inside the batch:
        >>> Point = namedtuple('Point', ['x', 'y'])
        >>> default_collate([Point(0, 0), Point(1, 1)])
        Point(x=array([0, 1]), y=array([0, 1]))
